...
Documentation is available in Chinese and English. The game interface is currently in Chinese.
...
Open <http://127.0.0.1:4173> in your browser and click **进入世界** (Enter world). Keep the terminal running; press `Ctrl+C` to stop the server.
...
See the [Blender guide](blender/README.en.md) for the generator and rebuild instructions. The Blender project is for modeling and rendering; use the browser game above to play.
...
tests/physics.test.ts    Physics and interaction boundary tests
...
Tests cover the spawn point, doorway, gravity, jumping, wall and ceiling collisions, ray hits, and placement boundaries. Basic movement and block interactions have been checked in a local browser. Touch controls have not yet been tested on a physical phone.
